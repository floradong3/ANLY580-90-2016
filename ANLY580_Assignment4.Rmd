...
## Ying Dong
Below is my example of using the mtcars datset to creat a heatmap that uses different colors to show the car characteristics by model. A heatmap is a graphical representation of data where the individual values contained in a matrix are represented in colors.  

Below is the heatmap vis created using the heatmap function and mtcars dataset with different options in the function to define the color and vis effect. 
```{r}
data(mtcars)
heatmap(as.matrix(mtcars), # numeric matrix of values to be plotted
Rowv=NA,  # determines if and how the row dendrogram should be computed and reordered
Colv=NA,  # determines if and how the column dendrogram should be computed and reordered
col = heat.colors(256), #type of color to be used in the heatmap
scale="column", #character indicating if the values should be centered and scaled in either the row direction or the column direction, or none. 
margins=c(2,8), #numeric vector of length 2 containing the margins (see par(mar = *)) for column and row names, respectively.
